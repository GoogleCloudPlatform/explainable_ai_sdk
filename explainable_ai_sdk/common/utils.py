...
class FieldKeys(object):
  ...
  @classmethod
  def values(cls):
    return {v for k, v in cls.__dict__.items() if not (
...
def split_feeds(dense_feed: types.TensorMap,
  ...
  sparse_sub_feeds = split_sparse_feed(sparse_feed, actual_size,
                                       max_batch_size)
  subbatch_count = actual_size // max_batch_size
  if actual_size % max_batch_size != 0:
    subbatch_count += 1
  return unify_subbatches(dense_feeds, sparse_sub_feeds, side_feeds,
                          subbatch_count)
...
def split_sparse_feed(sparse_feed: List[EvaluatedSparseTensor],
  ...
  sizes = [max_batch_size
           for _ in range(actual_size // max_batch_size)]
  if actual_size % max_batch_size > 0:
...
def _split_sparse_eval(sparse_eval: EvaluatedSparseTensor,
  ...
  indices_splits = _split_indices(
      indices_array, split_points_in_dense, split_at)
  ...
  for i, values_subbatch in enumerate(values_splits):
    feed_splits.append(
        EvaluatedSparseTensor({values_name: values_subbatch},
...
def _merge_subbatched_sparse_fetch(
  ...
  dense_shapes = [next(six.itervalues(sparse_eval.dense_shape))
...
def top_k_indices_for_batch(batched_array: np.ndarray, k: int = 1
    ...
    if len(batched_array.shape) == 2:
      return np.array(
          [np.unravel_index(np.argmax(array), array.shape)
           for array in batched_array], dtype=np.int32)
    else:
      return np.array(
          [[np.unravel_index(np.argmax(array), array.shape)]
           for array in batched_array], dtype=np.int32)
  ...
  for single_output in batched_array:
    top_k_batch.append(
        np.stack(
            np.unravel_index(
                single_output.argsort(axis=None)[-k:][::-1],
                single_output.shape),
            axis=1).squeeze())
