...
class MonkeyPatchUtilsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def setUpClass(cls):
    ...
    language_embedding = tf.feature_column.embedding_column(
    ...
    class_id_indicator = tf.feature_column.indicator_column(
        class_identity)
    ...
    cls.columns = [
        age, language_embedding, language_indicator, crossed_embedding,
        class_id_indicator, bucketized_age_column, embedded_bucketized_age
    ]
    ...
    cls.classifier_linear = tf.estimator.LinearClassifier(
        cls.base_columns)
  ...
  def test_exporting_context_no_output_key_regressor(self):
    ...
    with patcher.exporting_context():
      self.regressor.export_saved_model(
          tf.test.get_temp_dir(), self._get_json_serving_input_fn)
    self.assertIn(prediction_keys.PredictionKeys.PREDICTIONS,
