...
from explainable_ai_sdk.metadata.tf.v1 import estimator_metadata_builder


class EstimatorMetadataBuilderTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.class_id_indicator = tf.feature_column.indicator_column(
        cls.class_identity)
  ...
  def test_save_model_with_metadata_weighted_column(self):
    ...
    classifier_linear = tf.estimator.LinearClassifier(
        columns)
    classifier_linear.train(input_fn=self._train_input_fn, steps=5)
    md_builder = estimator_metadata_builder.EstimatorMetadataBuilder(
    ...
    expected_md = {
        'outputs': {
            'logits': {
                'output_tensor_name':
                    'linear/linear_model/linear_model/linear_model/'
                    'weighted_sum:0'
            }
        },
        'inputs': {
  ...
  def test_save_model_with_metadata_selected_columns(self):
    classifier_dnn = tf.estimator.DNNClassifier(
        hidden_units=[4],
        feature_columns=[
            self.age, self.language_indicator, self.language_embedding
        ])
    classifier_dnn.train(input_fn=self._train_input_fn, steps=5)
  ...
  def test_constructor_invalid_duplicate_treatment(self):
    with self.assertRaisesRegex(ValueError,
                                'Unrecognized treatment option for duplicates'):
      estimator_metadata_builder.EstimatorMetadataBuilder(
          tf.estimator.LinearClassifier([self.age, self.language]),
          [self.age, self.language],
          self._get_json_serving_input_fn,
          'logits',
          duplicate_feature_treatment='invalid')
  ...
  def test_save_model_with_metadata_duplica_treatment(
      self, treatment, expected_md):
    dnn_columns = [self.age, self.class_id_indicator]
    wide_columns = [self.age, self.class_identity]
    wide_deep_classifier = tf.estimator.DNNLinearCombinedClassifier(
        linear_feature_columns=wide_columns,
  ...
  def test_get_metadata(self):
    ...
    self._assertMetadataEqualsWithPrefix(expected_md, md_builder.get_metadata())
